...
- **Publicación autorizada por Emi el 25/09, pendiente de ejecutar:** Emi decidió publicar sin backup previo, porque Railway es demostrativo y sus datos se pueden recrear; el backup sigue siendo obligatorio antes del lanzamiento real (Fase 5). PM verificó `e9cf4c6` desde una base recién creada: suite 172/191 más 18/18 en cadena, con 190 de 191 casos cubiertos (el 169 es de entorno); a11y 76/76, contraste 84/84, auditoría 12/12, guía 26/26, build, tipos, lint, compilación y `alembic check` verdes. No hay archivos prohibidos ni secretos en la diferencia con `main`. El push a `main` lo bloqueó la política de permisos de la sesión de PM; lo ejecuta Emi o una sesión con ese permiso, y tiene que ser exactamente `e9cf4c6`. Railway corre `alembic upgrade head` como paso previo al despliegue.
