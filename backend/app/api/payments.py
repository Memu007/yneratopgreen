...
async def simulate_payment(
    ...
    Simula un pago aprobado o rechazado sin usar MP real.
    ...
    if order.status != OrderStatus.PLACED:
        raise HTTPException(status_code=400, detail="Esta orden no puede pagarse")
    ...
    if not payment:
        payment = Payment(
            order_id=order.id,
            mp_preference_id=f"SIM-{order.order_number}",
            mp_external_reference=order.order_number,
            total_amount=Decimal(str(total_amount)),
            commission_amount=Decimal(str(commission_amount)),
            commission_percent=Decimal(str(commission_percent)),
    ...
    if result == "approved":
        payment.status = PaymentStatus.APPROVED
        payment.paid_at = datetime.utcnow()
        payment.mp_payment_id = f"SIM-PAY-{order.order_number}"
        ...
        message = f"Pago simulado exitosamente. Comisión TopGreen: ${commission_amount} ({commission_percent}%), Vendedor recibe: ${seller_amount}"
    else:
        payment.status = PaymentStatus.REJECTED
        order.status = OrderStatus.CANCELLED
        order.cancellation_reason = "Pago rechazado (simulación)"
        # Restaurar stock (solo para productos, no servicios)
        for item in order.items:
            is_service = item.product.category.is_service if item.product and item.product.category else False
            if item.product and not is_service:
